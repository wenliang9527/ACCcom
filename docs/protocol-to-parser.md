...
## 路径 D：可视化编辑器（桌面 UI 操作）

无需手写 JSON 或代码，通过图形界面编辑协议字段，点击保存生成解析器。

### 第 1 步：打开编辑器

在桌面端工具栏找到 **「编辑器」** 按钮（解析器下拉框旁边），点击打开。
...
| Name | 协议名称，作为文件名 |
| Description | 可选描述 |
| Header | 帧头 Hex，如 `AA 55` |
| Footer | 帧尾 Hex，可选 |
| MinLength | 最小帧长度 |
| Checksum | 校验类型：none / xor8 / sum8 / crc16 |

### 第 3 步：添加字段

在字段 DataGrid 中，每行定义一个字段：

| 列 | 说明 |
|------|------|
| Offset | 字节偏移 |
| Name | 字段名称 |
| Len | 字节长度（根据 Type 自动调整） |
| Type | 数据类型（下拉选择：uint8 / uint16 / uint32 / int8 / int16 / int32 / float / double / string / hex / bcd / enum / bitfield） |
| Unit | 单位（如 °C, mV），可选 |
| BE | 大端模式（选中后 uint16/uint32/float/double 按大端解析） |
| Values | 枚举/位域值，格式 `0x01=温度,0x02=状态` |
| Color | 颜色代码（如 #22C55E），可选 |

点击 **"+ Add Field"** 添加行，**"×"** 删除， **"↑"/"↓"** 调整顺序。

### 第 4 步：预览并保存

底部的 TabControl 提供三个预览面板：

1. **JSON** — 自动生成的 Protocol Schema JSON
2. **.csx Code** — 自动生成的解析器脚本代码
3. **Test Parse** — 输入 Hex 数据测试解析结果

点击 **"Generate"** 刷新预览，**"Save"** 或 **"Save to parsers/"** 保存到 `parsers/` 目录并立即生效。

### 第 5 步：加载模板

点击 **"Load Template"** 加载内置模板（帧头+长度+命令码枚举+数据），在此基础上修改，快速开始。

---

## 多帧拼接重组

某些设备将一帧数据拆成多个包发送，需要将分片数据合并后再解析。

### 工作流程

```
分片1: AA 55 06      → 缓存
分片2: 01 02 03 04   → 缓存 + 长度满足 → 组装为 "AA 55 06 01 02 03 04" → 解析 → UI 显示
```

### 开启方式

1. 在桌面端工具栏点击 **「🔗」按钮**
2. 勾选 **"Enable multi-frame assembly"**
3. 配置参数：

| 参数 | 说明 |
|------|------|
| Frame Header | 帧头 Hex，用于识别帧开始 |
| Length Field Offset | 长度字段偏移（-1 = 不按长度匹配） |
| Length Field Size | 长度字段字节数（1 或 2） |
| Max Frame Size | 最大帧长，超过重置 |
| Timeout (ms) | 分片超时，超时未完成则丢弃 |

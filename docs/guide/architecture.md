...
| `SchemaEditorViewModel` | 协议可视化编辑器 |
| `FieldItemViewModel` | 编辑器字段条目 |
| `PortItemViewModel` | 端口条目 |
...
│   │   │   ├── FrameAssemblerConfig.cs  # 多帧拼接配置
...
│   │   │   ├── AutoParserMatcher.cs     # 自动解析器匹配 (指纹识别)
...
│   │   │   ├── FileExportService.cs     # 数据导出 (TXT/JSON/CSV/PCAP)
│   │   │   ├── PcapExportService.cs     # PCAP 格式导出
│   │   │   ├── DataStatistics.cs        # 数据统计
│   │   │   ├── TriggerService.cs        # 触发器系统
│   │   │   ├── HighlightService.cs      # 高亮规则引擎
│   │   │   ├── PacketFilter.cs          # 包过滤表达式引擎
│   │   │   ├── PatternMatcher.cs        # 统一模式匹配工具
│   │   │   ├── HexHelper.cs             # HEX 工具方法
│   │   │   ├── CrcHelper.cs             # CRC 计算工具
│   │   │   ├── ModbusScanner.cs         # Modbus 设备扫描器
...
│   │   │   ├── ModbusAsciiTransport.cs  # Modbus ASCII 传输层
...
│   │   ├── CreateSlaveDialog.xaml # 创建从站对话框
...
│   │   ├── PacketFilterTests.cs
│   │   ├── PatternMatcherTests.cs
│   │   ├── HighlightServiceTests.cs
│   │   ├── AutoBaudDetectorTests.cs
│   │   ├── AutoParserMatcherTests.cs
│   │   ├── VirtualSerialServiceTests.cs
│   │   ├── SerialServiceIntegrationTests.cs
│   │   ├── SerialConnectionManagerTests.cs
│   │   ├── NetworkBridgeServiceTests.cs
│   │   ├── MultiPortServiceTests.cs
│   │   ├── ModbusServiceTests.cs
│   │   ├── ModbusTcpTransportTests.cs
...
│   │   └── TestCollections.cs
│   └── ACCcom.McpServer.Tests/      # MCP 服务单元测试 (7 个文件, ~46 个测试)
│       ├── SerialToolsTests.cs
│       ├── ParserToolsTests.cs
│       ├── ModbusToolsTests.cs
